...
TAGLINE = "Total Cost of Ownership (TCO) comparison: Taiga vs Traditional Building (TRAD)"
BASE_PATH = Path(__file__).parent
LOGO_PATH = BASE_PATH / "logo.png" # korvaa omalla tiedostolla (esim. taiga_logo.png)
HERO_IMG = None # esim. BASE_PATH / "hero.jpg" – valinnainen


st.set_page_config(page_title=APP_TITLE, page_icon="🌲", layout="wide")


# ---------- LATAA CSS (turvallisesti) ----------
css_file = BASE_PATH / "style.css"
if css_file.exists():
    with open(css_file, "r", encoding="utf-8") as f:
        st.markdown(f"<style>{f.read()}</style>", unsafe_allow_html=True)
else:
    st.warning("style.css puuttuu – käytetään Streamlitin oletustyylejä.")
...
with col_logo:
    if LOGO_PATH.exists():
        # Yhteensopivuus eri Streamlit-versioiden kanssa
